...
let getAmount locale currency entry =
    ...
    if amount < 0.0 then
        ...
        | Nl, USD -> ("$ " + amount.ToString("#,#0.00", CultureInfo("nl-NL"))).PadLeft(13)
        | Nl, EUR -> ("€ " + amount.ToString("#,#0.00", CultureInfo("nl-NL"))).PadLeft(13)
        | En, USD -> ("($" + amount.ToString("#,#0.00", CultureInfo("en-US")).Substring(1) + ")").PadLeft(13)
        | En, EUR -> ("(€" + amount.ToString("#,#0.00", CultureInfo("en-US")).Substring(1) + ")").PadLeft(13)
    else
        match locale, currency with
        | Nl, USD -> ("$ " + amount.ToString("#,#0.00", CultureInfo("nl-NL")) + " ").PadLeft(13)
        | Nl, EUR -> ("€ " + amount.ToString("#,#0.00", CultureInfo("nl-NL")) + " ").PadLeft(13)
        | En, USD -> ("$" + amount.ToString("#,#0.00", CultureInfo("en-US")) + " ").PadLeft(13)
        | En, EUR -> ("€" + amount.ToString("#,#0.00", CultureInfo("en-US")) + " ").PadLeft(13)
